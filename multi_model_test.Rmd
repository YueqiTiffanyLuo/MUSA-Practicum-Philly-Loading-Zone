...
data <- read.csv("./updated_04_01_panel.csv")
...
LINEAR MIXED MODELS
```{r}
hist(data$bookings)  # seems close to a normal distribution - good!
```
```{r}
basic.lm <- lm(bookings ~ bodyLength2, data = dragons)
```
...
# The 'Day' column as a factor for group data
group_data <- as.factor(data$Day)
...
preds <- predict(
  gp_model,
  X_pred = X, # Replace or omit if predicting for new data
  group_data_pred = group_data, # Replace with new groups if predicting for new data
  predict_response = TRUE
)

# Access predicted values and variance
predicted_values <- preds$mu
predicted_variance <- preds$var
```

```{r}
X <- model.matrix(~week + Weekend_dummy + retail_nn1 + quick_grocery_nn2 + CLASS_2 + Bike_Network_dummy + parking_nn2 + school_nn3 + healthcare_nn3 + attraction_nn3 +dining_nn3 + housing_nn2 + commercial_nn2, data = data)

# The 'Day' column as factor for group data
group_data <- as.factor(data$Day)

# Assuming 'bookings' is count data and using "poisson" likelihood
likelihood <- "poisson"

# Fit the GPBoost model
gp_model <- fitGPModel(
  group_data = group_data,
  y = data$bookings,
  X = X, # Include the design matrix with multiple predictors
  likelihood = likelihood
)
...
# For predictions (example uses the same dataset)
preds <- predict(
  gp_model,
  X_pred = X, # The design matrix for predictions
...
n <- nrow(data)
indices <- sample(1:n, size = round(0.7 * n)) # 70% for training
...
X_train <- model.matrix(~ week + Weekend_dummy + retail_nn1 + quick_grocery_nn2 + CLASS_2 + Bike_Network_dummy + parking_nn2 + school_nn3 + healthcare_nn3 + attraction_nn3 +dining_nn3 + housing_nn2 + commercial_nn2, data = train_data)
y_train <- train_data$bookings
group_train <- as.factor(train_data$Day)

# For the test data
X_test <- model.matrix(~  week + Weekend_dummy + retail_nn1 + quick_grocery_nn2 + CLASS_2 + Bike_Network_dummy + parking_nn2 + school_nn3 + healthcare_nn3 + attraction_nn3 +dining_nn3 + housing_nn2 + commercial_nn2, data = test_data)
y_test <- test_data$bookings
group_test <- as.factor(test_data$Day)
...
gp_model <- fitGPModel(
  ...
  y = y_train,
  X = X_train,
  likelihood = "poisson" # Adjust based on your response variable
)

```

```{r}
# Calculate RMSE for a count (Poisson) response variable
rmse <- sqrt(mean((preds$mu - y_test)^2))
rmse
