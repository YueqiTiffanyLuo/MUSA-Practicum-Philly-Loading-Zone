...
The City of Philadelphia’s Smart Cities Director (of the City’s Innovation Management Team and Office of Information Technology) (“the client”) wishes to create a proof-of-concept data framework for siting new curbside loading zones for commercial vehicles. The increase in home delivery and on-demand logistics has created a need for new tools to decongest the right-of-way. In 2022-23, Philadelphia piloted a “Smart Loading Zone” program in select locations. This was an app-based, sensor-monitored reservation system. The pilot generated data about individual vehicles parking in each zone. What will happen if we want to site new loading zones elsewhere in the city? How can an official make an argument for taking away curb parking at a particular location? Your job is to use pilot data to create accurate models that can be used to estimate demand at new locations.
...
panel <- st_read("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/Data%20Used/Ling/all_curbs_nn.geojson")
...
simple_panel <- read.csv("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/Data%20Used/Ling/03_31_panel.csv")
detail_panel <- read.csv("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/Data%20Used/Ling/detailed_03_31_panel.csv")
old_panel <- read.csv("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/Data%20Used/Ling/updated_04_01_panel.csv")
panel <- read_sf("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/Data%20Used/Ling/morning_only_panel.geojson")
...
### Hyper Parameters
mtry=5, ntree = 3000, nodesize = 500,maxnodes = 20
...
set.seed(2024)
tuneGrid <- expand.grid(mtry = c(5,10,20,30,45))

rf_mtry <- train(Events~.,
...
store_nodesize <- list()
tuneGrid <- expand.grid(.mtry = best_mtry)
for (nodesize in c(50,100,200,500)) {
    set.seed(2024)
    rf_nodesize <- train(Events~.,
        data = train_set_simple,
        method = "rf",
        metric = "RMSE",
        tuneGrid = tuneGrid,
...
# ntree:3000
store_maxtrees <- list()
for (ntree in c(1000, 2000, 3000)) {
    set.seed(2024)
    rf_maxtrees <- train(Events~.,
        data = train_set_simple,
        method = "rf",
        metric = "RMSE",
        tuneGrid = tuneGrid,
...
The chart visualizes the relative importance of the top 20 variables in a predictive model. Notably, the 'distance to classes 1,4,5 roads' variables stand out as the most significant predictors, followed by the nearest healthcare facilities, indicating their substantial influence on the model's output. Other proximity-related variables such as civic,retail, dining, school, quick grocery, housing, and schools are also identified as key factors.
...
ImpDataTop <- ImpDataTop %>%
  mutate(Var.Names = factor(Var.Names, levels = Var.Names[order(Overall)]))

ggplot(ImpDataTop, aes(x = Var.Names, y = Overall)) +
  geom_segment(aes(x = Var.Names, xend = Var.Names, y = 0, yend = Overall), color = "#35B779") +
  geom_point(aes(y = Overall), color = "#482777", alpha = 0.6) +
  theme_light() +
  coord_flip() +
  theme(
...
#write_csv(test_set_simple,"./testresult_0420.csv")
```
...
test_set_simple %>%
  select(SmartZoneName, ordered_week, day_of_week, Events, Prediction) %>%
  gather(Variable, Value, Events, Prediction) %>%  # Transform data to long format for plotting
  group_by(Variable, ordered_week, day_of_week) %>%
...
error_analysis_day$day_of_week <- factor(error_analysis_day$day_of_week, 
                                 levels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"), 
                                 ordered = TRUE)


ggplot(error_analysis_day, aes(x = day_of_week, y = MAE)) +
...
ggplot(error_analysis_day, aes(x = factor(hour_of_day), y = MAE)) +
  geom_bar(stat = "identity", aes(fill = factor(hour_of_day))) +
  scale_fill_manual(values = palette5) + 
  labs(x = "Hour of the Day", y = "Mean Absolute Residual", 
       title = "Mean Absolute Residuals by Hour of the Day") +
  theme_minimal()
...
# should define a specific week, day, hour to see the predictions.
allPredictions <- 
  predict(fit_rf_simple, all_curbs)
