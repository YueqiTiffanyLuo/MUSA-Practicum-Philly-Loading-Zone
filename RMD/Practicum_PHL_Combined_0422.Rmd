...
panel <- st_read("https://raw.githubusercontent.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/main/04_23_reduced_panel.geojson")
...
# 设置交叉验证的训练控制
cv_folds <- vfold_cv(train_set_simple, v = 5)
...
final_rf_model <- finalize_workflow(
  ...
  last_fit(train_set_simple)

# 预测测试集
predictions <- predict(final_rf_model, test_set_simple)
...
train_set_simple<- train_set_simple%>%
  select(-SmartZoneName)
tuneGrid <- expand.grid(mtry = c(10,20,45))
...
ggplot(error_analysis_day, aes(x = factor(hour), y = MAE)) +
  geom_bar(stat = "identity", aes(fill = factor(hour))) +
...
for(day in c("Mon","Tue","Wed","Thu","Fri")) {
    for(hour in 6:9) {
      ...
      temp_predictions <- predict(fit_rf_simple, temp_data)
