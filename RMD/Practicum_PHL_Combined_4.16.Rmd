...
# Road Type/Bike Network Analysis for Panel 

```{r setup, include=FALSE}

# Read the GeoJSON file
booking_data <- st_read("C:/Users/YLuo/OneDrive - ARCADIS/Desktop/3.29EDA_Tiffany/Booking Data_cleaned.geojson")

completestreets <- st_read("C:/Users/YLuo/OneDrive - ARCADIS/Desktop/3.29EDA_Tiffany/CompleteStreets.geojson")

bike_network <- st_read("C:/Users/YLuo/OneDrive - ARCADIS/Desktop/3.29EDA_Tiffany/Bike_Network.geojson")

panel <- read_csv("C:/Users/YLuo/OneDrive - ARCADIS/Desktop/3.29EDA_Tiffany/03_31_panel.csv")
...
## Adding columns to the first panel file

# road type

```{r pressure, echo=FALSE}
booking_data <- booking_data %>%
  mutate(Chestnut_St = ifelse(grepl("Chestnut", StreetName, ignore.case = TRUE), 1, 0),
         Walnut_St = ifelse(grepl("Walnut", StreetName, ignore.case = TRUE), 1, 0),
         Sansom_St = ifelse(grepl("Sansom", StreetName, ignore.case = TRUE), 1, 0),
         Broad_St = ifelse(grepl("Broad", StreetName, ignore.case = TRUE), 1, 0),
         )
...
booking_summary <- booking_data %>%
  group_by(SmartZoneName) %>%
  summarise(Chestnut_St = max(Chestnut_St),
            Walnut_St = max(Walnut_St),
            Sansom_St = max(Sansom_St),
            Broad_St = max(Broad_St))
