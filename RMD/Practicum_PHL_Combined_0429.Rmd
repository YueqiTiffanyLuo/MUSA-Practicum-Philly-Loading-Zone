...
# load CDS event data
event_export <- read.csv("https://github.com/YueqiTiffanyLuo/MUSA-Practicum-Philly-Loading-Zone/blob/main/Data%20Used/Ling/events_export_CDS.csv", sep=",")
...
```{r clear NN table, fig.height=4, fig.width=8}
# Set tmap mode to view for interactive maps
tmap_mode("view")

# Plot the booking data and bike network
tm <- tm_shape(sf_buildings, name="OSM Buildings") +
...
```{r pressure, fig.height=4, fig.width=8}
# Set tmap mode to view for interactive maps
tmap_mode("view")

# Plot the booking data and bike network
tm <- tm_shape(curbs, name="Streets") +
...
tm+tm_view(set.view=c(-75.16,39.95,16))
...
ggcorrplot(
  ...
  insig = "blank",  # Don't show insignificant correlations
  #lab = TRUE,  # Show correlation coefficients in the plot
  #lab_size = 3.5,  # Adjust label size
  colors = c("#440154", "white", "#35B779"),  # Use a high-contrast color palette
  tl.cex = 8,  # Text size for variable names
  tl.col = "black",  # Text color for variable names
  tl.srt = 45  # Text rotation for variable names
) + 
  labs(title = 'Correlation across all variables') +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),  # Center the title
    axis.text.x = element_text(angle = 45, hjust = 1)  # Tilt x-axis labels for better visibility
  )
...
This map provides a spatial overview of the model’s performance across different streets, where larger circles with more bright colors indicate a higher MAPE at that curb, suggesting that the model's predictions are less accurate on Walnut St, while Chestnut & Sansom St has a relatively better performance.
...
tm <- tm_shape(error_analysis_curb) +
  tm_bubbles(size = "MAPE", col = "MAPE", palette = "viridis", scale=5, border.col = "black", 
             title.size = "MAPE", title.col = "MAPE") 
tm
```

From the chart, the high error in conjunction with high data counts suggest a systematic issue with the model especially on Friday and Tuesday, rather than a random fluctuation. As such, there's need to further check the weekly patterns or specific events impacting the model performance. While those low error points with low data counts, like Monday, demonstrate its good performance.
...
ggplot(error_analysis_day, aes(x = day, y = MAPE)) +
  geom_point(aes(size = Count, color = MAPE)) +  
  scale_color_gradient(low = "#35B779", high = "#440154") +  
  labs(x = "Day of the Week", 
       y = "Daily Mean Absolute Percentage Error (MAPE)", 
       title = "Daily Forecast Error as a Percentage") +
  theme_minimal() +
...
ggplot(error_analysis_day, aes(x = factor(day), y = MAPE)) +
  geom_bar(stat = "identity", aes(fill = factor(day))) +
  scale_fill_manual(values = palette5) + 
  labs(x = "Day of the Week", y = "Mean Absolute Percentage Error (MAPE)", 
       title = "Daily Forecast Error as a Percentage") +
  theme_minimal()

```

Overall, the model seems to be more accurate and precise for lower values of bookings and shows some divergence from the observed data at higher booking values. The similarity of the trend lines in five panels suggests the model performs consistently across weekdays and weekends. However, the model doesn't seem to be able to catch high values within the weekdays.
