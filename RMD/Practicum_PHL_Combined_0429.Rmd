...
```{r cars, echo=TRUE, fig.height=4, fig.width=4}
...
```{r cars, echo=TRUE, fig.height=4, fig.width=6}
...
```{r cars, echo=TRUE, fig.height=2, fig.width=2}
streets_of_interest <- c("Chestnut", "Walnut", "Sansom")
...
# 执行 left_join
all_results <- left_join(all_predictions_sf, all_curbs_sf[, c("wkt", "curb_id")], by = "wkt")
...
all_results_simplified <- all_results %>%
  select(day,week,curb_id,Prediction)
...
tm8 <- tm_shape(all_results_by_curb) +
  tm_lines(col = "Prediction_Sum", alpha = 0.5, legend.show=FALSE, id="strt_name.x", lwd=4) +
