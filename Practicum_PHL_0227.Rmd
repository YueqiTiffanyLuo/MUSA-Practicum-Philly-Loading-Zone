...
In certain locations during 2022-23, Philadelphia launched a trial run of the "Smart Loading Zone" initiative. This program, facilitated by a mobile app and sensors, allowed for reservations in specified zones where individual vehicle parking data was collected. The need for innovative solutions arose due to the increasing demand for home delivery and on-demand logistics, aiming to alleviate right-of-way congestion.

A Loading Zone, traditionally, provides dedicated street space for loading and unloading with a set time limit, and parking is not allowed. Usage rules are communicated through signages next to these zones. In contrast, a Smart Loading Zone serves the same purpose but stands out with its digitized regulations. These zones, bookable through the Pebble Driver app, offer a digitalized space, allowing delivery companies to reserve and pay for usage time under a pay-per-use fee structure.

Unlike conventional loading zones, Smart Loading Zones streamline the process through digitalized availability and regulations, with collaboration from Pebble by Sidewalk Labs. The City provides software applications for drivers, fleet managers, and parking enforcement.

To reserve a Smart Loading Zone, drivers download the Pebble Driver App, create an account, and add payment details. Fleet managers can efficiently manage multiple vehicles through the Pebble Fleet Manager Portal. Parking Enforcement Officers enforce regulations via the Pebble Inspector App, and transportation officials monitor metrics through the Smart Loading Zone Dashboard. Low-cost vehicle detection technology is deployed to enhance data reliability.

The Smart Loading Zone Pilot tested technologies related to loading activity in Philadelphia. The goals were to: 

1. Provide a reliable, efficient way for delivery drivers to safely use curb spaces. 
2. Manage increased loading activity. 
3. Reduce unsafe and illegal parking. 


The Smart Loading Zone Pilot addressed loading activity, aiming to provide reliable, efficient curb space for delivery, manage increased loading, and reduce illegal parking. Center City witnessed the introduction of 20 paid "smart loading zones." Companies reserved spaces through a smartphone app, and fleet managers registered on the Fleet Manager platform. Insights from the pilot inform potential program expansion and considerations for establishing new loading zones while justifying curb parking removal in other city areas.

The data collected from this pilot program offers interesting insight into reservation patterns that can be used for potential expansion of the program, answering the questions: 

What will happen if we want to site new loading zones elsewhere in the city? How can an official make an argument for taking away curb parking at a particular location? Where should new curbs go?
...
#Sam
#setwd(D:/Spring_2024/Practicum/resmartloadingzones")
...
coords <- st_as_sf(coords_raw, coords = c("longitude", "latitude"), crs = 'EPSG:4326')%>%st_transform('ESRI:102729') %>% select(-coords)
bld <- st_read('./LI_BUILDING_FOOTPRINTS.geojson')%>% filter(Shape__Area > 2000)
bldf <- bld%>% 
  st_transform('ESRI:102729')%>% 
  filter(Shape__Area > 4000)%>%
...
booking_only$curb_zone_id <- str_sub(booking_only$OMFCurbZoneID , start= -3)
# load curb zone data
...
event <- event_export%>%select(-event_time_start,-event_time_end,-event_session_id)
...
### Building footprint exploration

```{r}
# calculate the number of large building (size > 2000 sqft) in the 660 ft buffer of each loading zone 
coords$bld.Buffer <- coords %>% 
    st_buffer(660) %>% 
    aggregate(mutate(bldf, counter = 1),., sum) %>%
    pull(counter)

# calculate the number of distance to 5 nearest large building
coords <- coords %>% 
  mutate(bld_nn5 = nn_function(st_coordinates(st_centroid(coords)), st_coordinates(st_centroid(bldf)), 3))
```

```{r}
# join the curb zone id to coords dataset
zone_id <- booking_only %>% dplyr::select(geometry,curb_zone_id, SmartZoneName, SmartZoneNumber)%>%drop_na()%>%unique()
zone_id <- zone_id %>% st_drop_geometry() %>% select(-geometry)
coords <- left_join(coords,zone_id, by = 'SmartZoneName')
```


```{r}
# plot Distance to Large Building VS Parking Counts
df_long <- pivot_longer(coords, cols = c(bld.Buffer,bld_nn5,parking_count), names_to = "Group", values_to = "Value")
ggplot(df_long, aes(x = curb_zone_id, y = Value, fill = Group)) + 
  geom_bar(stat = "identity", position = "dodge") +  
  theme_minimal() + 
  scale_fill_viridis(discrete = TRUE, option = "D") +
  labs(x = "Category", y = "Value", title = "Distance to Large Building VS Parking Counts") 
```
