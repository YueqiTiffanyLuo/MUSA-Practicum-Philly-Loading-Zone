...
panel <- final_panel %>%
  select(-c("week", "car", "freight", "truck", "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat",
                "Mid_Day", "overnight", "PM_Rush", "Total_Dwell_Time", "Average_Dwell_Time", 
                "curb_zone_id", "van")) %>%
...
sample <- sample(c(TRUE, FALSE), nrow(panel), replace=TRUE, prob=c(0.7,0.3))
train  <- panel[sample, ]
test   <- panel[!sample, ]

model = train %>%
  select(-c("log_events","SmartZoneName","geometry.x","geometry.y")) %>%
  lm(Total_Events ~ ., data = .)
summary(model) #adj R^2 of 0.86
```


```{r}
panel1 <- final_panel %>%
  select(c("SmartZoneName","Total_Events","week", "car", "freight", "truck", "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat",
                "Mid_Day", "overnight", "PM_Rush", "Total_Dwell_Time", "Average_Dwell_Time", 
                "curb_zone_id", "van")) %>%
  mutate(log_events = log10(Total_Events))
...
#use 70% of dataset as training set and 30% as test set
sample1 <- sample(c(TRUE, FALSE), nrow(panel1), replace=TRUE, prob=c(0.7,0.3))
train1  <- panel1[sample1, ]
test1   <- panel1[!sample1, ]

model1 = train1 %>%
  select(-c("log_events","SmartZoneName","curb_zone_id")) %>%
  lm(Total_Events ~ ., data = .)
summary(model1) #adj R^2 of 1
