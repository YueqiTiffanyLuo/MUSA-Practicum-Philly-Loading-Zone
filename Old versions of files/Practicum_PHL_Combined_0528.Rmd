---
title: "Philadelphia Smart Loading Zones: Analysis and Demand Prediction"
author: "Ling Chen, Samriddhi Khare, Michael Dunst, Tiffany Luo, Shengqian Wang"
date: "2024-05-05"
output:
  html_document:
    toc: true
    ...
    dependencies:
      - name: htmlcompressor
        version: "0.1"
        src: "path/to/htmlcompressor"
        script: "htmlcompressor.js"
editor_options: 
  markdown: 
    wrap: 72
---
...
```{r utilization, echo=TRUE, message=FALSE}
...
```{r timeandviolation, echo=TRUE, fig.height=4, fig.width=8, message=FALSE}
...
```{r dwelltime_withviolation, echo=TRUE, fig.height=4, fig.width=8, message=FALSE}
...
```{r dwelltime_withoutviolation, echo=TRUE, fig.height=4, fig.width=8, message=FALSE}
...
```{r time_clean, message=FALSE, include=FALSE}
...
```{r exploratory_analysis_curb, fig.height=6, fig.width=8, message=FALSE}
...
```{r exploratory_serial, echo=TRUE, fig.height=5, fig.width=8}
#peak parking times
event_export$event_time_start <- as.POSIXct(event_export$event_time_start, format = "%Y-%m-%d %H:%M:%S")
...
```{r exploratory_operation, echo=TRUE, fig.height=3, fig.width=6}
...
```{r echo=TRUE, fig.height=4, fig.width=8, message=FALSE}
curb_bookings <- panel %>%
  group_by(SmartZoneName) %>%
...
```{r cars1, echo=TRUE, fig.height=4, fig.width=8}
...
```{r bike, fig.height=4, fig.width=8, message=FALSE}
# Set tmap mode to view for interactive maps
tmap_mode("view")
...
```{r nearest neighbors, fig.height=6, fig.width=9, message=FALSE}
...
```{r clear NN table, echo=TRUE, fig.height=6, fig.width=12}
...
```{r OSM, fig.height=4, fig.width=8, message=FALSE}
# Set tmap mode to view for interactive maps
tmap_mode("view")
...
```{r st_classification, fig.height=4, fig.width=8, message=FALSE}
# Set tmap mode to view for interactive maps
tmap_mode("view")
...
```{r road_NN, fig.height=4, fig.width=8, message=FALSE}
# Set tmap mode to view for interactive maps
tmap_mode("view")
...
```{r pressure, echo=FALSE, fig.height=12, fig.width=8}
print(new_cats)
```
...
```{r variable_importance, echo=TRUE, fig.height=4, fig.width=8}
...
```{r echo=TRUE, fig.height=4, fig.width=6, message=TRUE}
...
```{r roc, echo=TRUE, fig.height=4, fig.width=4, message=TRUE}
# ROC curve
...
```{r cv, echo=TRUE, fig.height=4, fig.width=8, message=FALSE}
setwd("/Users/lingchen/Documents/Practicum/Practicum_Philly_2024/Data")
...
In the diagram illustrating the demand for smart loading zones in Philadelphia, several patterns emerge. Notably, there's a consistent high demand observed near Independence Mall, likely due to its status as a popular tourist destination and the presence of numerous historical sites, leading to increased commercial activity and delivery needs. Conversely, the lack of demand near highways could be attributed to the limited accessibility and the focus of commercial activity in more centralized areas. The spikes in demand midweek near universities are likely influenced by the rhythms of student life, with increased deliveries for campus facilities and student-oriented businesses. Additionally, the high demand near commercial streets can be explained by the concentration of businesses and retail establishments, necessitating frequent deliveries to meet consumer demands. These trends provide valuable insights for optimizing the placement and allocation of smart loading zones to efficiently meet the city's logistical needs.
...
all_predictions_16 <- all_predictions %>%
  unnest(Prediction) %>%
  rename(Prediction=.pred)

#write_csv(all_predictions,"./allcurbs_prediction_16_0506.csv")
```

```{r export_result, echo=TRUE, message=FALSE}

all_curbs_sf <- st_as_sf(all_curbs_2,wkt="geometry")
all_predictions_sf <- st_as_sf(all_predictions_16,wkt="geometry")
...
all_results <- all_results %>% select(-wkt)  

#write_csv(all_results,"./allcurbs_detailed_0505.csv")
```

```{r simplified_panel, echo=TRUE}
...
```{r EDA export, eval=FALSE, include=FALSE}
#EDA export
...
```{r allcurbs_pred, echo=TRUE, fig.height=4, fig.width=8, message=FALSE}

#setwd("/Users/lingchen/Documents/Practicum/Practicum_Philly_2024/Data")
#all_results_simplified_2<-st_read("./allcurbs_simplified_0505.geojson")

all_results_simplified$date <- as.Date(all_results_simplified$date, format = "%Y-%m-%d")

pred_1_3 <- all_results_simplified %>%
  filter(date >= as.Date("2025-01-01") & date <= as.Date("2025-3-31"))

Pred_1_3_by_curb <- pred_1_3 %>%
  group_by(curb_id,day) %>%
  summarize(Prediction_Sum = sum(Prediction, na.rm = TRUE),
            Pred_binary_sum=sum(Prediction_binary,na.rm=TRUE))

# 假设你知道正确的CRS是NAD83 / Pennsylvania South (ftUS)，其EPSG代码为 2272
Pred_1_3_by_curb <- st_set_crs(Pred_1_3_by_curb, 2272)

```

```{r pred_all_1_3, echo=TRUE, fig.height=4, fig.width=9, message=FALSE}

data <- Pred_1_3_by_curb %>%
  filter(day %in% c("Mon", "Tue", "Wed", "Thu", "Fri")) %>%

  # 按curb_id和周聚合数据
  group_by(curb_id) %>%
  summarize(
    Mon_sum = sum(Pred_binary_sum[day == "Mon"], na.rm = TRUE),
    Tue_sum = sum(Pred_binary_sum[day == "Tue"], na.rm = TRUE),
    Wed_sum = sum(Pred_binary_sum[day == "Wed"], na.rm = TRUE),
    Thu_sum = sum(Pred_binary_sum[day == "Thu"], na.rm = TRUE),
    Fri_sum = sum(Pred_binary_sum[day == "Fri"], na.rm = TRUE),
    .groups = 'drop'
  )

tmap_mode("view")

tm9 <- tm_shape(data, name="Monday") +
  tm_lines(col = "Mon_sum", alpha = 0.5, legend.show = TRUE, id = "curb_id", lwd = 4) +
  tm_shape(data, name="Tuesday") +
  tm_lines(col = "Tue_sum", alpha = 0.5, legend.show = FALSE, id = "curb_id", lwd = 4) +
  tm_shape(data, name="Wednesday") +
  tm_lines(col = "Wed_sum", alpha = 0.5, legend.show = FALSE, id = "curb_id", lwd = 4) +
  tm_shape(data, name="Thursday") +
  tm_lines(col = "Thu_sum", alpha = 0.5, legend.show = FALSE, id = "curb_id", lwd = 4) +
  tm_shape(data, name="Friday") +
  tm_lines(col = "Fri_sum", alpha = 0.5, legend.show = FALSE, id = "curb_id", lwd = 4) +
  tm_layout(title = "Weekly Prediction Summary by Curbs, from January to March, 2025")

tm9 + tm_view(set.view=14)
